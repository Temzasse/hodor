...
const ruleId = "hodor/no-leaked-resize-observer";
const { collectRuleMessages, runOxlint } = createOxlintRuleTester({
  ruleDirectoryUrl: import.meta.url,
  tempProjectPrefix: "hodor-no-leaked-resize-observer-",
});

describe("hodor/no-leaked-resize-observer via oxlint CLI", () => {
  it("does not report valid code", () => {
    const run = runOxlint({ fixtureName: "valid-no-leaked-resize-observer.tsx" });
  ...
  it("reports invalid code", () => {
    const run = runOxlint({ fixtureName: "invalid-no-leaked-resize-observer.tsx" });
    const messages = collectRuleMessages(run.parsed, ruleId);
