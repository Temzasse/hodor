...
const ruleId = "hodor/no-render";
const { collectRuleMessages, runOxlint } = createOxlintRuleTester({
  ruleDirectoryUrl: import.meta.url,
  tempProjectPrefix: "hodor-no-render-",
});

describe("hodor/no-render via oxlint CLI", () => {
  it("does not report valid code", () => {
    const run = runOxlint({ fixtureName: "valid-no-render.tsx" });
  ...
  it("reports invalid code", () => {
    const run = runOxlint({ fixtureName: "invalid-no-render.tsx" });
    const messages = collectRuleMessages(run.parsed, ruleId);

    expect(run.status).toBe(1);
    expect(messages).toHaveLength(1);
    expect(messages[0]?.message).toContain("createRoot");
  });
});
